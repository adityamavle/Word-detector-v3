...
class MixedSampler(Sampler[T_co]):
    def __init__(self, dataset1: Dataset, dataset2: Dataset, num_replicas: Optional[int] = None,
                 rank: Optional[int] = None, seed: int = 0, drop_last: bool = False) -> None:
        ...
        if self.drop_last and len(self.dataset1) % self.num_replicas != 0:  # type: ignore[arg-type]
            # Split to nearest available length that is evenly divisible.
            # This is to ensure each rank receives the same amount of data when
            # using this Sampler.
            self.num_samples = math.ceil(
                (len(self.dataset1) - self.num_replicas) / self.num_replicas  # type: ignore[arg-type]
            )
        else:
            self.num_samples = math.ceil(len(self.dataset1) / self.num_replicas)
        # self.num_samples = math.ceil(len(self.dataset1) / self.num_replicas)
    ...
    def get_indices(self, dataset: Dataset, num_samples: int) -> List[int]:
...
def main(args):
    ...
    easy_train_set = DetectionDataset(
        ...
        img_transforms=Compose(
            [
                ...
                ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.02), #add more color augmentations here
            ]
        ),
        sample_transforms=T.SampleCompose(
            (
    ...
    #old_train_set
    old_train_set = DetectionDataset(
            img_folder=os.path.join(args.old_data_train_path, "images"),
            label_path=os.path.join(args.old_data_train_path, "labels.json"),
            img_transforms=Compose(
                [
                    # Augmentations
                    T.RandomApply(T.ColorInversion(), 0.1),
    ...
    N_olddata = len(old_train_set) #old_consort_data
    e = len(easy_train_set)
    # m = len(medium_train_set)
    h = len(hard_train_set)
    N = e+h
    #newmedium  = e* (N-m)/(N-e)
    newhard = e* (N-h)/(N-e) #almost 5 times of old e #newhard
    print("size of Easy,Newhard, hard ,Total docs (all for train set): ",e,newhard,h,N)
    ...
    print("length of the new hard and easy train set: ",len(new_hard_train_set),len(easy_train_set))
    newdata_train_set = torch.utils.data.ConcatDataset([easy_train_set,new_hard_train_set]) #combined new_data
    
    Nnew = e + len(new_hard_train_set)
    ...
    #resampling for 70-30 split
    Nnew_newdata = Nnew*((N_olddata*7)/(Nnew*3)) #new increased number of samples in newdata to match the 70-30 split in minibatch

    ####write code here to increase the number of samplesin newdata to match 70-30 split in mininbatch

    new_newdata_train_set = copy.deepcopy(newdata_train_set)
    while(len(new_newdata_train_set) + len(newdata_train_set) < Nnew_newdata):
        new_newdata_train_set = torch.utils.data.ConcatDataset([new_newdata_train_set,newdata_train_set])
    tail_end_size = math.floor(Nnew_newdata - len(new_newdata_train_set))
    tail_end, extra = torch.utils.data.random_split(newdata_train_set,[tail_end_size, len(newdata_train_set)-tail_end_size])
    new_newdata_train_set = torch.utils.data.ConcatDataset([new_newdata_train_set,tail_end])

    
    #new_newdata_train_set -> train set for new data after resampling for 70-30 split in batch
    #old_train_set -> train set for old data i.e. consort data. This does not require any resampling
    # we then combine both the datasets as a single combined dataset, 
    # and define our new mixed sampler to sample 70% of samples in a batch from dataset1 and the other 30% of samples in a batch from dataset2
    ###################
    ...
    combined_dataset = torch.utils.data.ConcatDataset([old_train_set,new_newdata_train_set])
    mixed_sampler = MixedSampler(old_train_set,new_newdata_train_set,shuffle = True)
    ...
    #changed
    with open(os.path.join(args.easy_train_path, "labels.json"), "rb") as f: #replace the train_paths_here
        easy_train_hash = hashlib.sha256(f.read()).hexdigest()

    # with open(os.path.join(args.medium_train_path, "labels.json"), "rb") as f: #delete this
    #     medium_train_hash = hashlib.sha256(f.read()).hexdigest()

    with open(os.path.join(args.hard_train_path, "labels.json"), "rb") as f: #replace the train_paths for hard images here
...
def parse_args():
    ...
    parser.add_argument("--hard_train_path", type=str,
                        default="/scratch/sreevatsa/data/train/Hard", help="path to training data folder")  # c
    
    parser.add_argument("--val_path", type=str, default="/scratch/sreevatsa/data/val",
                        help="path to validation data folder")
    parser.add_argument("--test_path", type=str,
                        default="/scratch/sreevatsa/data/test", help="path to test data folder")

    parser.add_argument("--old_data_train_path", type=str,
                        default="/scratch/sreevatsa/consort_hard/train", help="path to training data folder") 
    parser.add_argument("--old_data_test_path", type=str,
                        default="/scratch/sreevatsa/consort_hard/test", help="path to training data folder") 
    parser.add_argument("--old_data_val_path", type=str,
                        default="/scratch/sreevatsa/consort_hard/val", help="path to training data folder") 
    ...
    # DDP configs:
    parser.add_argument('--world-size', default=-1, type=int,   #figure out distributed training 
