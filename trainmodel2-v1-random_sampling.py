...
def fit_one_epoch(model, train_loader, batch_transforms, optimizer, scheduler, mb):  # one epoch function
    ...
    for images, targets in progress_bar(train_loader, parent=mb):
        ...
        # set gradient of optimized tensors to 0,before computing next batch of data.
        optimizer.zero_grad()
        train_loss = model(images, targets)["loss"]
        train_loss.backward()
        # clipping gradients to make sure gradients do not exceed 5.
...
def main(args):
    ...
    print(
        f"test set loaded in {time.time() - st:.4}s ({len(test_set)} samples in " f"{len(test_loader)} batches)")
    ...
    batch_transforms = Normalize(
        mean=(0.798, 0.785, 0.772), std=(0.264, 0.2749, 0.287))
    # batch_transforms = Normalize(mean=(0.5,), std=(1.,))

    # Load doctr model
    model = detection.__dict__["db_resnet50"](
        pretrained=args.pretrained, assume_straight_pages=True)
    model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)
    local_rank = int(os.environ['LOCAL_RANK'])

    # Backbone freezing
    if args.freeze_backbone:
        # freeze_backbone=True;model....(False)->freezing
        model.feat_extractor.requires_grad_(False)
    ...
    model = torch.nn.parallel.DistributedDataParallel(
        model, device_ids=[local_rank]) #use when multi GPUs
    ...
    print("Current GPU memory usage (in bytes):", torch.cuda.memory_allocated())
    print("Maximum GPU memory usage (in bytes):",
    ...
    if args.test_only:
        ...
        print(
            f"Validation loss: {val_loss} (Recall: {recall} | Precision: {precision} | Mean IoU: {mean_iou})")

        print("Running evaluation for test")
        test_loss, recall, precision, mean_iou = evaluate(
            model, test_loader, batch_transforms, test_metric)
        print(
            f"test loss: {test_loss} (Recall: {recall} | Precision: {precision} | Mean IoU: {mean_iou})")
        return
    ...
    #Loading the 3 train sets 
    #to perform class balanced training by oversampling of hard or medium images
    ...
    train_set = torch.utils.data.ConcatDataset(
        [easy_train_set, medium_train_set, hard_train_set])
    ...
    for epoch in mb:
        train_loss = fit_one_epoch(
            model, train_loader, batch_transforms, optimizer, scheduler, mb)
        ...
        # print("b0  -  ",val_loss)
        world_size = int(os.environ['WORLD_SIZE'])
        train_loss, val_loss, recall, precision, mean_iou = train_loss/world_size, val_loss / \
            world_size, recall/world_size, precision/world_size, mean_iou/world_size
        ...
        if dist.get_rank() == 0:
            torch.save(model.state_dict(),
                       f"./models/{exp_name}_epoch{epoch}.pt")
            ...
            if counter == patience:
                print("Validation loss hasn't improved in",
...
def parse_args():
    ...
    parser.add_argument("--easy_train_path", type=str,
                        default="/scratch/sreevatsa/scratch/abhaynew/newfolder/train/Easy", help="path to training data folder")
    parser.add_argument("--medium_train_path", type=str,
                        default="/scratch/sreevatsa/scratch/abhaynew/newfolder/train/Medium", help="path to training data folder")
    parser.add_argument("--hard_train_path", type=str,
                        default="/scratch/sreevatsa/scratch/abhaynew/newfolder/train/Hard", help="path to training data folder")

    parser.add_argument("--val_path", type=str, default="/scratch/sreevatsa/scratch/abhaynew/newfolder/val",
                        help="path to validation data folder")
    parser.add_argument("--test_path", type=str,
                        default="/scratch/sreevatsa/scratch/abhaynew/newfolder/test", help="path to test data folder")
    parser.add_argument("--name", type=str, default=None,
                        help="Name of your training experiment")
    parser.add_argument("--epochs", type=int, default=10,
                        help="number of epochs to train the model on")
    parser.add_argument("-b", "--batch_size", type=int,
                        default=9, help="batch size for training")
    parser.add_argument("--device", default=None, type=int, help="device")
    parser.add_argument("--input_size", type=int,
                        default=512, help="model input size, H = W")
    parser.add_argument("--lr", type=float, default=0.001,
                        help="learning rate for the optimizer (Adam)")
    parser.add_argument("--wd", "--weight-decay", default=0,
    ...
    parser.add_argument("--test-only", dest="test_only",
                        action="store_true", help="Run the validation loop")
    parser.add_argument(
    ...
    parser.add_argument("--push-to-hub", dest="push_to_hub",
                        action="store_true", help="Push to Huggingface Hub")
    parser.add_argument(
        "--pretrained",
        dest="pretrained",
        action="store_true",
