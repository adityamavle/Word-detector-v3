...
def main(args):
    ...
    N = e+h
    #newmedium  = e* (N-m)/(N-e)
    newhard = e * (N-h)/(N-e)  # almost 5 times of old e
    print("size of Easy,Newhard, hard ,Total docs (all for train set): ",
          e, newhard, h, N)

    new_hard_train_set = copy.deepcopy(hard_train_set)
    while(len(new_hard_train_set) + h < newhard):
        new_hard_train_set = torch.utils.data.ConcatDataset(
            [new_hard_train_set, hard_train_set])
    tail_end_size = math.floor(newhard - len(new_hard_train_set))
    tail_end, extra = torch.utils.data.random_split(
        hard_train_set, [tail_end_size, h - tail_end_size])
    new_hard_train_set = torch.utils.data.ConcatDataset(
        [new_hard_train_set, tail_end])
    ...
    print("length of the new hard and easy train set: ",
          len(new_hard_train_set), len(easy_train_set))
    train_set = torch.utils.data.ConcatDataset(
        [easy_train_set, new_hard_train_set])
    ...
    # replace the train_paths for hard images here
    ...
    for epoch in mb:
        ...
        if dist.get_rank() == 0:
            ...
            if any(val is 0 for val in (recall, precision, mean_iou)):
...
def parse_args():
    ...
    parser.add_argument("--easy_train_path", type=str,
                        default="/scratch/sreevatsa/new_data/train/Easy", help="path to training data folder")  # c
...
# add hard image paths
    ...
    parser.add_argument("--test_path", type=str,
                        default="/scratch/sreevatsa/new_data/test", help="path to test data folder")
